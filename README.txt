...
If you experience troubles with speed, you can alter the speed by doing the following:
  ...
  1. Go into agentsim.py and change the running speed (self._speed) in the do_run() method (line 198). Setting self._speed to a value of 120 is recommended
  2. Change score delay (line 58) in the do_step() function helicopter_game.py . The recommended delay value to use is 30 
  
  In most cases, changing the above two settings is enough to fix speed issues 
  However, if you still have speed troubles, you can also change the heli_speed, and canvas_speed variables (line 8 and line 9) in helicopter_game.py
